...
# ── QIP Comparison Analysis ───────────────────────────────────────────────────
def _compute_period_stats(df: pd.DataFrame, mask: pd.Series) -> dict:
    sub = df[mask].copy()
    if sub.empty:
        return {
    ...
    failures = len(sub)
    # the column might be duration_min (from messy_loader/new data_loader) or downtime_minutes (from tsdpl_demo)
    dt_col = "duration_min" if "duration_min" in sub.columns else "downtime_minutes"
    total_dt = sub[dt_col].sum() if dt_col in sub.columns else 0.0
    mttr = total_dt / failures if failures else 0.0

    period_hours = (sub["timestamp"].max() - sub["timestamp"].min()).total_seconds() / 3600
    period_hours = max(period_hours, 1.0)

    uptime_hours = period_hours - (total_dt / 60)
    mtbf = uptime_hours / failures if failures else uptime_hours
...
def compute_qip_comparison(df: pd.DataFrame, base_start, base_end, impr_start, impr_end) -> dict:
    """
    Slice the data into two timeframes (baseline and improvement),
    calculate MTTR/MTBF for both, and return the percentage improvements.
    """
    if df.empty or "timestamp" not in df.columns:
        return {"baseline": {}, "improvement": {}, "deltas": {}}
        
    # Convert dates to datetime if they aren't already
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    
    b_mask = (df["timestamp"].dt.date >= base_start) & (df["timestamp"].dt.date <= base_end)
    i_mask = (df["timestamp"].dt.date >= impr_start) & (df["timestamp"].dt.date <= impr_end)
    
    b_stats = _compute_period_stats(df, b_mask)
    i_stats = _compute_period_stats(df, i_mask)
    ...
    deltas = {
        "mtbf": pct_change(i_stats["mtbf_h"], b_stats["mtbf_h"]),
        "mttr": pct_change(i_stats["mttr_min"], b_stats["mttr_min"]),
        "failures": pct_change(i_stats["failures"], b_stats["failures"]),
        "downtime": pct_change(i_stats["total_downtime_min"], b_stats["total_downtime_min"])
    }
