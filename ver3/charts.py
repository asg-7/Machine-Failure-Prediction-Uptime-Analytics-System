...
# ── UI CHANGE: Updated risk colours to TSDPL brand ───────────────────────────
RISK_COLORS = {
    "High":    "#C0392B",   # Alert Red
    "Medium":  "#FF9900",   # Tata Yellow
    "Low":     "#009900",   # Success Green
    "Unknown": "#5C6F87",   # Muted Text
}

# ── UI CHANGE: Brand-aligned qualitative palette (replaces px.colors.qualitative.Plotly) ──
PALETTE = [
    "#0080C7",  # Tata Blue
    "#FF9900",  # Tata Yellow
    "#009900",  # Success Green
    "#C0392B",  # Alert Red
    "#5C6F87",  # Muted Text / Steel
    "#1A2C3E",  # Deep Steel Blue
]

# ── UI CHANGE: Replaced dark-theme Plotly template with TSDPL light theme ────
import plotly.io as pio

TSDPL_TEMPLATE = {
    "layout": {
        # Light backgrounds — transparent so the app's #F4F7F9 shows through
        "paper_bgcolor": "rgba(0,0,0,0)",
        "plot_bgcolor":  "rgba(0,0,0,0)",
        # Primary text in Deep Steel Blue
        "font": {
            "color":  "#1A2C3E",
        ...
        # Tooltip styled with brand Dark Steel header
        "hoverlabel": {
            "bgcolor":    "#1A2C3E",
            "bordercolor":"#0080C7",
            "font":       {"color": "#F4F7F9"},
...
def chart_uptime_downtime(uptime_df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color="#009900",   # UI CHANGE: Success Green (was #24A148)
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color="#C0392B",   # UI CHANGE: Alert Red (was #DA1E28)
...
def chart_daily_uptime_trend(daily_uptime: pd.DataFrame, selected_machines=None) -> go.Figure:
    ...
    fig = px.line(
        ...
        color_discrete_sequence=PALETTE,   # UI CHANGE: brand palette
    )
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)",
        paper_bgcolor="rgba(0,0,0,0)",
    ...
    # UI CHANGE: target line in Muted Text (was #94A3B8)
    fig.add_hline(
        y=90,
        line_dash="dash",
        line_color="#5C6F87",
...
def chart_failure_frequency(failure_patterns: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(go.Bar(
        ...
        y=fp["machine_id"],
        orientation="h",
        marker_color="#0080C7",   # UI CHANGE: Tata Blue (was #0F62FE)
...
def chart_hourly_failure_trend(hourly_df: pd.DataFrame) -> go.Figure:
    fig = go.Figure(go.Bar(
        ...
        marker_color="#FF9900",   # UI CHANGE: Tata Yellow (was #F1C21B)
    ))
    fig.update_layout(
...
def chart_risk_ranking(predictions: pd.DataFrame) -> go.Figure:
    ...
    colors = [RISK_COLORS.get(r, "#5C6F87") for r in pred["risk_label"]]
    ...
    # UI CHANGE: threshold line colours updated to brand yelllow / red
    fig.add_vline(x=40, line_dash="dot", line_color="#FF9900", annotation_text="Medium")
    fig.add_vline(x=70, line_dash="dot", line_color="#C0392B", annotation_text="High")
    return fig
...
def chart_ttf_prediction(predictions: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Bar(
        ...
        y=pred["machine_id"],
        orientation="h",
        marker_color="#5C6F87",   # UI CHANGE: Muted Text (was #94A3B8)
    ))
    fig.add_trace(go.Bar(
...
def chart_sensor_trend(df_machine: pd.DataFrame, sensor: str, machine_id: str) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", name="UP",
        line=dict(color="#009900", width=1),   # UI CHANGE: Success Green
    ))
    fig.add_trace(go.Scatter(
        x=down["timestamp"], y=down[sensor],
        mode="markers", name="DOWN (failure)",
        marker=dict(color="#C0392B", size=5),  # UI CHANGE: Alert Red
    ))
    # Rolling mean
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color="#0080C7", dash="dash", width=2),  # UI CHANGE: Tata Blue
    ))
    fig.update_layout(
