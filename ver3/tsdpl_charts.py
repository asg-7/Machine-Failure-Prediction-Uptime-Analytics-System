...
─────────────────────────────────────────────────────────────────────
UI CHANGE: Replaced dark-theme colour palette with TSDPL brand colours.
  Primary Blue  #0080C7 | Tata Yellow  #FF9900
  Success Green #009900 | Alert Red    #C0392B
  Deep Steel    #1A2C3E | Muted Text   #5C6F87
  Background    #F4F7F9 | Card White   #FFFFFF
"""
...
# ── UI CHANGE: All colour maps updated to TSDPL brand palette ────────────────
RISK_COLOR = {
    "Critical": "#C0392B",   # Alert Red       (was #DA1E28)
    "High":     "#FF9900",   # Tata Yellow     (was #F1C21B)
    "Medium":   "#0080C7",   # Tata Blue       (was #0F62FE)
    "Low":      "#009900",   # Success Green   (was #24A148)
}

STATUS_COLOR = {
    "ALARM":     "#C0392B",  # Alert Red       (was #DA1E28)
    "WARNING":   "#FF9900",  # Tata Yellow     (was #F1C21B)
    "Normal":    "#009900",  # Success Green   (was #24A148)
    "Scheduled": "#5C6F87",  # Muted Text      (was #94A3B8)
    "Overdue":   "#C0392B",  # Alert Red       (was #DA1E28)
    "Completed": "#009900",  # Success Green   (was #24A148)
}

# UI CHANGE: Shift colours updated — Morning=Primary Blue, Afternoon=Yellow, Night=Deep Steel
SHIFT_COLORS = {
    "A - Morning":   "#0080C7",   # Tata Blue       (was #1192E8)
    "B - Afternoon": "#FF9900",   # Tata Yellow     (was #8A3FFC)
    "C - Night":     "#1A2C3E",   # Deep Steel Blue (was #002D9C)
}

TREND_COLOR = {
    "Improving": "#009900",  # Success Green (was #24A148)
    "Worsening": "#C0392B",  # Alert Red     (was #DA1E28)
    "Stable":    "#5C6F87",  # Muted Text    (was #94A3B8)
}

# ── UI CHANGE: Replaced dark Plotly template with TSDPL light brand theme ────
import plotly.io as pio

TSDPL_TEMPLATE = {
    "layout": {
        # Transparent backgrounds — app's #F4F7F9 shows through
        "paper_bgcolor": "rgba(0,0,0,0)",
        "plot_bgcolor":  "rgba(0,0,0,0)",
        # Primary text colour: Deep Steel Blue
        "font": {
            "color":  "#1A2C3E",
            "family": "Inter, system-ui, sans-serif",
        },
        # Brand-colour data-series order
        "colorway": ["#0080C7", "#FF9900", "#009900", "#C0392B", "#5C6F87", "#1A2C3E"],
        # Light grid / axis lines
        "xaxis": {
            "gridcolor": "#E5E7EB",
            "linecolor": "#272727",
            "tickcolor": "#5C6F87",
        ...
        # Tooltip styled with Deep Steel Blue header
        "hoverlabel": {
            "bgcolor":     "#1A2C3E",
            "bordercolor": "#0080C7",
            "font":        {"color": "#F4F7F9"},
...
def chart_shift_tornado(shift_mtbf: pd.DataFrame, machines: list = None) -> go.Figure:
    ...
    if morning_col:
        fig.add_trace(go.Bar(
            ...
            y=pivot["machine_id"],
            orientation="h",
            marker_color=SHIFT_COLORS["A - Morning"],   # UI CHANGE
    ...
    if afternoon_col:
        fig.add_trace(go.Bar(
            ...
            y=pivot["machine_id"],
            orientation="h",
            marker_color=SHIFT_COLORS["B - Afternoon"],  # UI CHANGE
    ...
    if night_col:
        fig.add_trace(go.Bar(
            ...
            y=pivot["machine_id"],
            orientation="h",
            marker_color=SHIFT_COLORS["C - Night"],      # UI CHANGE
    ...
    # UI CHANGE: centre-line in Steel Grey (was #1E293B dark)
    fig.add_vline(x=0, line_color="#272727", line_width=2)
    return fig
...
def chart_mtbf_by_shift(shift_mtbf: pd.DataFrame, metric: str = "MTBF_hours") -> go.Figure:
    ...
    # UI CHANGE: color_discrete_map uses updated SHIFT_COLORS
    fig = px.bar(
        shift_mtbf,
        x="machine_id",
        y=metric,
...
def chart_sensor_heatmap(scorecard_df: pd.DataFrame) -> go.Figure:
    ...
    # UI CHANGE: heatmap colour bands updated to brand Success / Yellow / Alert
    colorscale = [
        [0.0, "#D4EDDA"],   # Normal  — light Success Green tint
        [0.5, "#FFF3CD"],   # Warning — light Tata Yellow tint
        [1.0, "#F8D7DA"],   # Alarm   — light Alert Red tint
    ]
...
def chart_param_trend(sensor_df: pd.DataFrame, machine: str, param: str) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color="#0080C7", width=2),   # UI CHANGE: Tata Blue (was #0F62FE)
    ))
    # Rolling average
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color="#FF9900", dash="dash", width=2),  # UI CHANGE: Tata Yellow (was #F1C21B)
    ))
    if ah:
        fig.add_hline(
            y=ah, line_dash="dash", line_color="#C0392B",   # UI CHANGE: Alert Red
            annotation_text=f"Alarm High: {ah}", annotation_position="top right",
        )
    if al:
        fig.add_hline(
            y=al, line_dash="dash", line_color="#C0392B",   # UI CHANGE: Alert Red
...
def chart_failure_category_donut(downtime_log: pd.DataFrame, machine: str = None) -> go.Figure:
    ...
    # UI CHANGE: failure category colours mapped to TSDPL brand palette
    colors = {
        "Mechanical":              "#C0392B",  # Alert Red
        "Electrical":              "#0080C7",  # Tata Blue
        "Hydraulic/Pneumatic":     "#5C6F87",  # Muted Text
        "Performance Degradation": "#FF9900",  # Tata Yellow
        "Intermittent":            "#1A2C3E",  # Deep Steel Blue
        "Surface Degradation":     "#272727",  # Steel Grey
        "Process":                 "#009900",  # Success Green
        "Planned":                 "#5C6F87",  # Muted Text
    }
    c = [colors.get(cat, "#5C6F87") for cat in cat_counts["failure_category"]]
...
def chart_mom_grid(mom_df: pd.DataFrame) -> go.Figure:
    ...
    for idx, (_, row) in enumerate(mom_df.iterrows()):
        ...
        # UI CHANGE: MoM bar colours updated to TSDPL brand
        LAST_MONTH_COLOR = "#5C6F87"   # Muted Text       (was dark #334155)
        IMPROVING_COLOR  = "#009900"   # Success Green    (was #22c55e)
        WORSENING_COLOR  = "#C0392B"   # Alert Red        (was #ef4444)
        STABLE_COLOR     = "#0080C7"   # Tata Blue        (was #38bdf8)
...
def chart_pm_status(pm_df: pd.DataFrame) -> go.Figure:
    ...
    # UI CHANGE: PM status colours updated to TSDPL brand
    colors = {
        "Completed": "#009900",  # Success Green (was #24A148)
        "Overdue":   "#C0392B",  # Alert Red     (was #DA1E28)
        "Scheduled": "#0080C7",  # Tata Blue     (was #0F62FE)
    }
...
def chart_incident_timeline(downtime_log: pd.DataFrame, days_back: int = 14) -> go.Figure:
    ...
    # UI CHANGE: incident category colours updated to TSDPL brand palette
    category_color = {
        "Mechanical":              "#C0392B",  # Alert Red
        "Electrical":              "#0080C7",  # Tata Blue
        "Hydraulic/Pneumatic":     "#5C6F87",  # Muted Text
    ...
    for _, row in df.iterrows():
        color = category_color.get(row["failure_category"], "#5C6F87")
        fig.add_trace(go.Scatter(
...
# ── QIP Analysis Charts ───────────────────────────────────────────────────────
def chart_qip_comparison(
    baseline_stats: dict,
    improvement_stats: dict,
    b_label: str = "Baseline",
    i_label: str = "Improvement",
):
    """
    Grouped bar chart: Baseline vs Improvement for MTBF, MTTR, Failures, Downtime.
    """
    if not baseline_stats or not improvement_stats:
        return go.Figure()

    labels   = ["MTBF (hours)", "MTTR (min)", "Failures", "Downtime (min)"]
    b_values = [
        baseline_stats.get("mtbf_h", 0),
        baseline_stats.get("mttr_min", 0),
        baseline_stats.get("failures", 0),
        baseline_stats.get("total_downtime_min", 0),
    ]
    i_values = [
        improvement_stats.get("mtbf_h", 0),
        improvement_stats.get("mttr_min", 0),
        improvement_stats.get("failures", 0),
        improvement_stats.get("total_downtime_min", 0),
    ]
    ...
    fig = go.Figure(data=[
        go.Bar(
            name=b_label,
            x=labels, y=b_values,
            marker_color="#0080C7",   # UI CHANGE: Tata Blue (was #38bdf8 cyan)
        ...
        go.Bar(
            name=i_label,
            x=labels, y=i_values,
            marker_color="#009900",   # UI CHANGE: Success Green (was #4ade80 neon green)
