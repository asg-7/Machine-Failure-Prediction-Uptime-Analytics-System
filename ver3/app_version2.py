"""
app_version2.py  —  TSDPL Kalinganagar Maintenance & Operations Dashboard
12-Tab Streamlit App   |   Run: streamlit run app_version2.py
...
<style>
/* -- App shell ------------------------------- */
[data-testid="stAppViewContainer"] { background-color: #F4F7F9; color: #1A2C3E; }
[data-testid="stSidebar"] { background-color: #1A2C3E; border-right: 1px solid #272727; }

/* -- Sidebar: force ALL text white ----------------------------------------- */
[data-testid="stSidebar"] * { color: #FFFFFF !important; }

/* -- Sidebar: file uploader — nuclear dark override at EVERY nesting level -- */
...
[data-testid="stSidebar"] [data-testid="stFileUploadDropzone"] {
    border: 1px dashed #0080C7 !important;
    border-radius: 8px !important;
}
/* "Browse files" button — force visible at every wrapper depth */
...
/* Caption text below uploaders ("Limit 200MB per file") */
...
/* -- Cards / metrics -------------------------- */
[data-testid="stMetric"] { background: #FFFFFF; border: 1px solid #E5E7EB; border-left: 5px solid #0080C7; border-radius: 10px; padding: 12px 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
[data-testid="stMetricLabel"] { color: #5C6F87; font-size: 11px; }
[data-testid="stMetricValue"] { color: #0080C7; }
...
/* -- Buttons ---------------------------------- */
.stButton > button { background: #0080C7; color: #FFFFFF; border: 1px solid #0080C7; border-radius: 6px; font-weight: 500; transition: background 0.15s, transform 0.1s; }
.stButton > button:hover { background: #FF9900; color: #1A2C3E; border-color: #FF9900; transform: translateY(-1px); }

/* -- Alert / info boxes ----------------------- */
[data-testid="stAlert"][kind="error"] { background: #FEE2E2; border: 1px solid #C0392B; color: #7B1010; border-radius: 6px; }
[data-testid="stAlert"][kind="warning"] { background: #FFF3CD; border: 1px solid #FF9900; color: #7A4500; border-radius: 6px; }
[data-testid="stAlert"][kind="success"] { background: #D4EDDA; border: 1px solid #009900; color: #1A5C1A; border-radius: 6px; }

/* -- Table ------------------------------------ */
[data-testid="stDataFrame"] { border: 1px solid #E5E7EB; border-radius: 8px; }
thead th { background: #1A2C3E !important; color: #FFFFFF !important; }
tbody tr:hover td { background: #E5F4FF !important; }
...
/* ============================================================
   Fix selectbox / multiselect in MAIN AREA
   Root cause: Streamlit applies secondaryBackgroundColor (#1A2C3E)
   as an inline style on the select control div, making text invisible.
   We must override the control container background AND all text.
   ============================================================ */

/* The outer control box that gets the dark inline background */
.main [data-baseweb="select"] > div:first-child,
.main [data-baseweb="select"] > div:first-child > div,
.main [data-baseweb="select"] [class*="ValueContainer"],
.main [data-baseweb="select"] [class*="SingleValue"],
.main [data-baseweb="select"] [class*="Placeholder"],
.main [data-baseweb="select"] [class*="Input"],
.main [data-baseweb="select"] input {
    background-color: #FFFFFF !important;
    color: #1A2C3E !important;
}

/* The clickable select wrapper itself */
...
/* Dropdown popover option list (renders outside main, so no .main prefix) */
...
# -- JS: Override Streamlit's inline secondaryBackgroundColor on main-area select widgets --
# config.toml sets secondaryBackgroundColor=#1A2C3E which Streamlit injects as inline style
# on select controls — CSS !important cannot beat inline styles, so we use a MutationObserver.
st.markdown("""
<script>
(function fixSelectBg() {
    var LIGHT_BG  = "#FFFFFF";
    var LIGHT_TXT = "#1A2C3E";

    function patchNodes(root) {
        ...
        var selects = root.querySelectorAll(
            ...
            '.main [data-baseweb="select"] [class*="SingleValue"],' +
            '.main [data-baseweb="select"] [class*="Placeholder"],' +
            '.main [data-baseweb="select"] [class*="Input"] > div'
        );
        for (var i = 0; i < selects.length; i++) {
            selects[i].style.setProperty("background-color", LIGHT_BG,  "important");
            selects[i].style.setProperty("color",            LIGHT_TXT, "important");
        }
    }
    ...
    var observer = new MutationObserver(function(mutations) {
        for (var i = 0; i < mutations.length; i++) {
            ...
            for (var j = 0; j < m.addedNodes.length; j++) {
                if (m.addedNodes[j].nodeType === 1) patchNodes(m.addedNodes[j]);
            }
            if (m.target) patchNodes(m.target);
        }
    });
...
with st.sidebar:
    ...
    # -- Breakdown Excel uploader (Maintenance Delay Analysis) -----------------
    st.markdown("---")
    st.markdown("### ⚙️ Maintenance Delay Analysis")
    st.caption(
        "Upload one or more Breakdown Excel files (WCTL-1, WCTL-2, Slit). "
        "All files are merged automatically."
    )

    breakdown_files = st.file_uploader(
        "Breakdown Excel file(s)",
        type=["xlsx"],
        accept_multiple_files=True,
        key="breakdown_upload",
    )

    if breakdown_files:
        try:
            from utils.breakdown_loader import load_breakdown_excel

            all_raws = []
            last_hr_col = None

            for f in breakdown_files:
                data = load_breakdown_excel(f)
                all_raws.append(data['raw_df'])
                last_hr_col = data['_hr_col']

            combined_raw = pd.concat(all_raws, ignore_index=True)
            hr_col = last_hr_col

            combined_pareto = (
                combined_raw.groupby('EQUIPMENT')[hr_col]
                .agg(['sum', 'count'])
                .reset_index()
            )
            combined_pareto.columns = ['EQUIPMENT', 'BREAKDOWN(HRs)', 'BREAKDOWN(No.s)']
            combined_pareto = combined_pareto.sort_values(
                'BREAKDOWN(HRs)', ascending=False
            ...
            combined_pivot.columns = [
                'EQUIPMENT', 'Sum of Unplanned B/D (Hr)', 'Count of Unplanned B/D'
            ]
            combined_pivot = combined_pivot.sort_values(
                'Sum of Unplanned B/D (Hr)', ascending=False
            ).reset_index(drop=True)

            st.session_state['breakdown_data'] = {
                'raw_df':         combined_raw,
                'pareto_df':      combined_pareto,
                'pivot_df':       combined_pivot,
                'line_list':      sorted(combined_raw['AREA'].dropna().unique().tolist()),
                'equipment_list': sorted(combined_raw['EQUIPMENT'].dropna().unique().tolist()),
                '_hr_col':        hr_col,
            }

            lines_loaded = sorted(combined_raw['AREA'].dropna().unique().tolist())
            st.success(
                f"✅ {len(breakdown_files)} file(s) loaded\n"
                f"Lines: {', '.join(lines_loaded)}\n"
                f"{len(combined_raw):,} total events"
            )
        except Exception as e:
            logger.error(f"Breakdown loader error: {e}", exc_info=True)
            st.error("❌ Error loading breakdown file(s). Check format and try again.")
            # FIX 2: Always reset to None on error
            st.session_state['breakdown_data'] = None
    else:
        # FIX 2: Unconditionally reset when no files are selected.
        # Original code guarded this with 'if not in session_state', which meant
        # removing files from the uploader kept stale data alive indefinitely.
        st.session_state['breakdown_data'] = None
    ...
    st.markdown("---")
    st.markdown(
        '<div style="font-size:.72em;color:#F4F7F9;text-align:center;">'
        'TSDPL Kalinganagar v2.0<br>Jajpur Road, Odisha</div>',
        unsafe_allow_html=True,
    )


# -- Landing page --------------------------------------------------------------
# FIX 1: Original condition only checked generic + TSDPL data. If a user uploaded
# only a breakdown file, st.stop() fired before st.tabs() was called, making
# tab_bd undefined and the breakdown tab completely inaccessible.
# Fix: include breakdown_data in the gate condition.
_no_data = (
    not st.session_state.tsdpl_loaded
    and st.session_state.df is None
    and st.session_state.get('breakdown_data') is None
)

if _no_data:
    st.markdown("""
    <div style="background-color: #0080C7; padding: 20px; border-radius: 12px;
        ...
        <p style="color: #FF9900; margin-top: 12px;">
            ⚡ Predictive maintenance · Shift analytics · Sensor health
        </p>
    </div>
    """, unsafe_allow_html=True)

    # FIX 5 (UX): Added breakdown tile so users know the tab exists on first load
    tiles = [
        ...
        ("⚙️", "Maintenance Delay",        "Equipment Pareto from Breakdown Excel"),
    ]
    for row_start in range(0, len(tiles), 3):
        ...
        for col, (icon, title, desc) in zip(cols, tiles[row_start:row_start+3]):
            with col:
                st.markdown(
                    f"""<div style="background:#FFFFFF;border:1px solid #E5E7EB;border-left: 4px solid #0080C7;border-radius:10px;
                    padding:18px;text-align:center;margin-bottom:8px">
                    <div style="font-size:2em">{icon}</div>
                    <strong style="color:#1A2C3E;">{title}</strong>
                    <p style="color:#5C6F87;font-size:.85em;margin-top:6px">{desc}</p>
                    </div>""",
                    unsafe_allow_html=True,
                )
    st.stop()
...
(tab_ov, tab_up, tab_fa, tab_pr, tab_dr,
 tab_ro, tab_sa, tab_pm_tab, tab_sc, tab_mm, tab_qip, tab_bd) = st.tabs([
    "🏠 Overview", "📈 Uptime", "💥 Failures", "🔮 Predictions", "🔬 Drilldown",
    "📋 Shift Roster", "📊 Shift Analytics", "🔧 PM Checklist", "🌡️ Health Scorecard",
    "📅 Month-over-Month", "📊 QIP Analysis",
    "⚙️ Maintenance Delay Analysis",
])
...
def render_generic(df_all):
    ...
    with tab_ov:
        ...
        with c2: metric_card("Avg Uptime", f"{avg_up:.1f}%", color="#009900" if avg_up >= 90 else "#FF9900")
        with c3: metric_card("Total Failures", str(len(fe)), color="#C0392B")
        with c4: metric_card("High Risk", str(high_r), color="#C0392B" if high_r > 0 else "#009900")
        col1, col2 = st.columns(2)
        with col1:
    ...
    with tab_up:
        ...
        if not ut.empty:
            ...
            with c1: metric_card("Best Performer",  best,  delta=f"{ut.loc[best,'uptime_pct']}%",  color="#009900")
            with c2: metric_card("Needs Attention", worst, delta=f"{ut.loc[worst,'uptime_pct']}%", color="#C0392B")
            with c3: metric_card("Total Downtime", f"{ut['downtime_hours'].sum():.0f} h", color="#FF9900")
            c1, c2 = st.columns(2)
            with c1:
    ...
    with tab_fa:
        ...
        if not fe.empty:
            c1, c2, c3 = st.columns(3)
            with c1: metric_card("Total Failures", str(len(fe)), color="#C0392B")
            ...
            fe_d = fe.copy()
            fe_d["failure_time"] = fe_d["failure_time"].dt.strftime("%Y-%m-%d %H:%M")
    ...
    with tab_dr:
        ...
        if sel:
            ...
            if fe_m.empty:
                alert_banner("No failures recorded.", "info")
            else:
                st.dataframe(fe_m.drop(columns=["machine_id"]), use_container_width=True)
...
def render_tsdpl(tsdpl):
  ...
  *(MasterRoster[Zone]=[@Zone])*(MasterRoster[Role]="Operator"),
  MasterRoster[Name], "Standby")""", language="excel")
        ...
        total_s = len(rv)
        inc_s   = (rv["incident_logged"] == "Y").sum()
        c1, c2, c3 = st.columns(3)
        with c1: metric_card("Shift Slots",    f"{total_s:,}")
        with c2: metric_card("With Incidents", str(inc_s), color="#C0392B")
        with c3: metric_card("Incident Rate",  f"{100*inc_s/max(total_s,1):.1f}%", color="#FF9900")
        ...
        if not inc_df.empty:
            st.dataframe(
                inc_df.groupby("operator_name").size().reset_index(name="incident_count")
    ...
    with tab_sa:
        ...
        if shift_mtbf.empty:
            ...
            c1, c2, c3 = st.columns(3)
            for col, sh, clr in zip([c1, c2, c3], SHIFT_CODES, ["#0080C7","#FF9900","#1A2C3E"]):
                with col:
...
- **Recommendation:** Mandate per-coil knife inspection sign-off on Night shift; add Cassette Leveller ΔT alarm from 22:00.
    ...
    with tab_pm_tab:
        ...
        with c2: metric_card("Completed",      str((pm_view["status"] == "Completed").sum()), color="#009900")
        with c3: metric_card("Overdue",        str((pm_view["status"] == "Overdue").sum()),   color="#C0392B")
        with c4: metric_card("LOTO Required",  str(int(pm_view["loto_required"].sum())),      color="#FF9900")
        ...
        disp["status"] = disp["status"].map({
            "Completed": "✅ Completed", "Overdue": "🔴 Overdue", "Scheduled": "📅 Scheduled"
        }).fillna(disp["status"])
...
```
    ...
    with tab_sc:
        ...
        if scorecard.empty:
            ...
            if not alarms.empty:
                alarm_str = "; ".join(
                    f"{r['machine_id']} / {r['parameter']} = {r['current_value']} {r['unit']}"
                    for _, r in alarms.iterrows()
                )
                alert_banner(f"🚨 <strong>ACTIVE ALARMS ({len(alarms)}):</strong> {alarm_str}", "error")
            if not warnings.empty:
                warn_str = "; ".join(
                    f"{r['machine_id']} / {r['parameter']}" for _, r in warnings.iterrows()
                )
            ...
            with c2: metric_card("In Alarm",   str(len(alarms)),   color="#C0392B")
            with c3: metric_card("In Warning", str(len(warnings)), color="#FF9900")
            ...
            for m, md in MACHINES.items():
                for p, cfg in md["params"].items():
                    lim_rows.append({
                        "Machine": m, "Line": md["line"], "Criticality": md["criticality"],
            ...
            crit_map = {
                ...
                "Medium":   "background-color:#EFF6FF",
            }
            lim_df = pd.DataFrame(lim_rows)
            st.dataframe(
                lim_df.style.map(lambda v: crit_map.get(v, ""), subset=["Criticality"]),
                use_container_width=True,
            )
    ...
    with tab_mm:
        ...
        st.markdown(
            "<p style='color: #5C6F87; font-size: 13px; margin-top: -8px;'>"
        ...
        if mom_df.empty:
            ...
            def mom_kpi_card(label, value, border_color):
                return f"""
                <div style="background: #FFFFFF; border: 1px solid #E5E7EB; border-left: 4px solid {border_color}; border-radius: 6px; padding: 14px 20px;">
                  <p style="color: #5C6F87; font-size: 10px; letter-spacing: 0.08em;
                            text-transform: uppercase; margin: 0 0 6px;">{label}</p>
                  <p style="color: #1A2C3E; font-size: 30px; font-weight: 700;
                            margin: 0; line-height: 1;">{value}</p>
                </div>
                """

            c1, c2, c3, c4 = st.columns(4)
            with c1: st.markdown(mom_kpi_card("IMPROVING", str(imp), "#009900"), unsafe_allow_html=True)
            with c2: st.markdown(mom_kpi_card("WORSENING", str(wor), "#C0392B"), unsafe_allow_html=True)
            with c3: st.markdown(mom_kpi_card("STABLE",    str(sta), "#0080C7"), unsafe_allow_html=True)
            with c4:
                fleet_val = f"{'▲' if delta_total > 0 else '▼'} {abs(delta_total):,} min"
                st.markdown(mom_kpi_card("FLEET Δ DOWNTIME", fleet_val, "#FF9900"), unsafe_allow_html=True)
            ...
            for chunk_s in range(0, len(mom_df), 3):
                ...
                for col, (_, row) in zip(cols, chunk.iterrows()):
                    with col:
                        ...
                        bd = "#009900" if t == "Improving" else ("#C0392B" if t == "Worsening" else "#E5E7EB")
                        st.markdown(f"""<div style="background:{bg};border:1px solid {bd};border-radius:10px;
                        padding:14px;margin-bottom:10px">
                        <div style="font-weight:700;font-size:.85em;color:#1A2C3E;margin-bottom:8px">{row['machine_id']}</div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:.78em">
                        <div><span style="color:#5C6F87">MTBF This</span><br><strong>{row['mtbf_this_month_h']:.1f} h</strong></div>
                        <div><span style="color:#5C6F87">MTBF Last</span><br><strong>{row['mtbf_last_month_h']:.1f} h</strong></div>
                        <div><span style="color:#5C6F87">Failures</span><br><strong>{row['this_month_failures']}</strong>
                             <span style="color:#F4F7F9"> vs {row['last_month_failures']}</span></div>
                        <div><span style="color:#5C6F87">Top Failure</span><br><strong>{row['top_failure_mode']}</strong></div>
...
```
            ...
            cat_bg = {
                "Mechanical": "background-color:#FEF2F2",
                "Electrical": "background-color:#EFF6FF",
                "Hydraulic":  "background-color:#FFFBEB",
                "Process":    "background-color:#F0FDF4",
                "Planned":    "background-color:#F8FAFC",
            }
            st.dataframe(
                gloss.style.map(lambda v: cat_bg.get(v, ""), subset=["Category"]),
                use_container_width=True,
            )


# ═══════════════════════════════════════════════════════════════════════════════
# QIP ANALYSIS TAB (Tab 11)
# ═══════════════════════════════════════════════════════════════════════════════
def render_qip_tab(norm_df):
    ...
    with tab_qip:
        ...
        selected_equip = st.multiselect(
            ...
            all_equip, default=[], key="qip_equip",
        )
        ...
        c1, c2, c3, c4 = st.columns(4)
        mtbf_color = "#009900" if i_stats["mtbf_h"]            >= b_stats["mtbf_h"]            else "#C0392B"
        mttr_color = "#009900" if i_stats["mttr_min"]          <= b_stats["mttr_min"]          else "#C0392B"
        fail_color = "#009900" if i_stats["failures"]           <= b_stats["failures"]           else "#C0392B"
        dt_color   = "#009900" if i_stats["total_downtime_min"] <= b_stats["total_downtime_min"] else "#C0392B"

        with c1: metric_card("MTBF Change",    mtbf_delta, delta=f"B: {b_stats['mtbf_h']}h → I: {i_stats['mtbf_h']}h",          color=mtbf_color)
        with c2: metric_card("MTTR Change",    mttr_delta, delta=f"B: {b_stats['mttr_min']}m → I: {i_stats['mttr_min']}m",       color=mttr_color)
        with c3: metric_card("Failure Count",  fail_delta, delta=f"B: {b_stats['failures']} → I: {i_stats['failures']}",         color=fail_color)
        with c4: metric_card("Total Downtime", dt_delta,   delta=f"B: {b_stats['total_downtime_min']}m → I: {i_stats['total_downtime_min']}m", color=dt_color)
        ...
        fig = go.Figure(data=[
            go.Bar(name=f"Baseline ({b_start} → {b_end})",
                   x=labels, y=b_values, marker_color="#0080C7",
                   text=[str(v) for v in b_values], textposition="outside"),
            go.Bar(name=f"Improvement ({i_start} → {i_end})",
                   x=labels, y=i_values, marker_color="#009900",
                   text=[str(v) for v in i_values], textposition="outside"),
        ])
        fig.update_layout(
            barmode="group", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#1A2C3E"),
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 12 — MAINTENANCE DELAY ANALYSIS
# Isolated: no overlap with any other tab.
# Source  : Breakdown Details Excel (WCTL-1 / WCTL-2 / Slit / any line)
# ═══════════════════════════════════════════════════════════════════════════════
def render_breakdown_tab():
    ...
    with tab_bd:
        st.markdown("## ⚙️ Equipment-wise Maintenance Delay Analysis")
        st.caption(
            "Source: Breakdown Details Excel — upload via sidebar "
            "(supports WCTL-1, WCTL-2, Slit, or any line)"
        )

        bd = st.session_state.get('breakdown_data')

        if bd is None:
            st.info(
                "👈 Upload one or more **Breakdown Excel files** using the sidebar "
                "(**⚙️ Maintenance Delay Analysis** section) to activate this tab.\n\n"
                "You can upload WCTL-1, WCTL-2, and Slit files together — "
                "they will be merged automatically."
            )
            # FIX 3: Use return instead of st.stop().
            # st.stop() raises StopException and kills the entire script.
            # return exits only this function cleanly.
            return
        ...
        col_line, col_equip = st.columns(2)

        with col_line:
            selected_lines = st.multiselect(
                "📌 Line (AREA)",
                options=bd['line_list'],
                default=bd['line_list'],
                help="Filter by production line — e.g. WCTL-1, WCTL-2, Slit",
                key="bd_line_filter",
            )

        raw_filtered = (
            raw_df[raw_df['AREA'].isin(selected_lines)].copy()
            if selected_lines else raw_df.copy()
        )

        equip_available = sorted(raw_filtered['EQUIPMENT'].dropna().unique().tolist())

        with col_equip:
            selected_equipment = st.multiselect(
                "🔧 Equipment",
                options=equip_available,
                default=[],
                help="Leave blank to show all equipment for the selected lines",
                key="bd_equip_filter",
            )

        if selected_equipment:
            raw_filtered = raw_filtered[raw_filtered['EQUIPMENT'].isin(selected_equipment)]

        st.markdown("---")

        if raw_filtered.empty:
            st.warning("⚠️ No data matches the selected filters. Try broadening your selection.")
            # FIX 3: return instead of st.stop()
            return

        # -- KPI row -----------------------------------------------------------
        total_events = len(raw_filtered)
        total_hours  = raw_filtered[hr_col].sum()
        unique_equip = raw_filtered['EQUIPMENT'].nunique()
        avg_hrs      = total_hours / total_events if total_events > 0 else 0

        k1, k2, k3, k4 = st.columns(4)
        with k1: metric_card("Total Breakdown Events", f"{total_events:,}")
        with k2: metric_card("Total Downtime (Hrs)",   f"{total_hours:.1f}", color="#C0392B")
        with k3: metric_card("Equipment Affected",      f"{unique_equip}",   color="#FF9900")
        with k4: metric_card("Avg Hrs / Event",         f"{avg_hrs:.2f}",    color="#0080C7")

        st.markdown("---")

        # -- Pareto chart (hours + cumulative %) ------------------------------
        st.subheader("📊 Pareto — Breakdown Hours by Equipment")

        pareto = (
            raw_filtered.groupby('EQUIPMENT')[hr_col]
            .agg(['sum', 'count'])
            .reset_index()
        )
        pareto.columns = ['EQUIPMENT', 'BREAKDOWN(HRs)', 'BREAKDOWN(No.s)']
        # Sort by hours for Pareto (correct)
        pareto = pareto.sort_values('BREAKDOWN(HRs)', ascending=False).reset_index(drop=True)

        pareto_top = pareto.head(15).copy()
        pareto_top['Cumulative %'] = (
            pareto_top['BREAKDOWN(HRs)'].cumsum()
            / pareto_top['BREAKDOWN(HRs)'].sum() * 100
        ).round(1)

        fig_pareto = go.Figure()
        fig_pareto.add_trace(go.Bar(
            x=pareto_top['EQUIPMENT'],
            y=pareto_top['BREAKDOWN(HRs)'],
            name='Breakdown Hours',
            marker_color='#C0392B',
            text=pareto_top['BREAKDOWN(HRs)'].round(2),
        ...
        fig_pareto.add_trace(go.Scatter(
            ...
            name='Cumulative %',
            mode='lines+markers',
            marker=dict(size=6, color='#0080C7'),
            line=dict(color='#0080C7', width=2),
            yaxis='y2',
        ))
        fig_pareto.update_layout(
            title="Top Equipment — Breakdown Hours (Pareto)",
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#1A2C3E"),
            xaxis=dict(title='Equipment', tickangle=-35, gridcolor="#E5E7EB"),
            yaxis=dict(title='Breakdown Hours', side='left', gridcolor="#E5E7EB"),
            yaxis2=dict(
                title='Cumulative %', side='right', overlaying='y',
                range=[0, 110], ticksuffix='%', gridcolor="#E5E7EB",
            ),
            legend=dict(
                orientation='h', yanchor='bottom', y=1.02,
                xanchor='right', x=1, bgcolor="#FFFFFF", bordercolor="#E5E7EB",
            ),
            height=480, margin=dict(b=130),
        )
        st.plotly_chart(fig_pareto, use_container_width=True)

        with st.expander("📋 Full Pareto Table", expanded=False):
            st.dataframe(
                pareto.style.format({
                    'BREAKDOWN(HRs)': '{:.2f}', 'BREAKDOWN(No.s)': '{:.0f}'
                }),
        ...
        # -- Breakdown frequency chart (sorted by COUNT, not hours) ------------
        # FIX 4: Original code used pareto.head(15) which is sorted by hours.
        # A frequency chart must be sorted by event count to be meaningful.
        st.subheader("📈 Breakdown Frequency — Number of Events by Equipment")

        # Build a separate df sorted by count
        freq = pareto.sort_values('BREAKDOWN(No.s)', ascending=False).head(15)

        fig_count = px.bar(
            freq,
            x='EQUIPMENT', y='BREAKDOWN(No.s)',
            text='BREAKDOWN(No.s)',
            title="Number of Breakdown Events — Top 15 Equipment (by count)",
            labels={'BREAKDOWN(No.s)': 'Count of Events', 'EQUIPMENT': 'Equipment'},
            color='BREAKDOWN(No.s)',
        ...
        fig_count.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#1A2C3E"),
            xaxis=dict(tickangle=-35, gridcolor="#E5E7EB"),
            yaxis=dict(gridcolor="#E5E7EB"),
            height=420, margin=dict(b=130),
        ...
        # -- Pivot summary table -----------------------------------------------
        st.subheader("📋 Pivot Summary — Total Hours & Count by Equipment")
        ...
        pivot_summary.columns = ['EQUIPMENT', 'Sum of Unplanned B/D (Hr)', 'Count of Unplanned B/D']
        pivot_summary = pivot_summary.sort_values('Sum of Unplanned B/D (Hr)', ascending=False)

        st.dataframe(
            pivot_summary.style
                .format({
                    'Sum of Unplanned B/D (Hr)': '{:.2f}',
                    'Count of Unplanned B/D':    '{:.0f}',
                })
                .background_gradient(subset=['Sum of Unplanned B/D (Hr)'], cmap='Oranges')
                .background_gradient(subset=['Count of Unplanned B/D'],    cmap='Blues'),
            use_container_width=True, height=380,
        )

        st.markdown("---")

        # -- Raw breakdown events (collapsible) --------------------------------
        with st.expander("🔍 View Raw Breakdown Events", expanded=False):
            priority = [
                'DATE', 'MONTH', 'AREA', 'EQUIPMENT',
                'UNPLANNED B/D (Min.)', hr_col,
                'BRIEF DESCRIPTION OF MAJOR UNPLANNED BREAKDOWNS',
            ]
            available = [c for c in priority if c in raw_filtered.columns]
            remaining = [c for c in raw_filtered.columns if c not in available]
            show_cols = available + remaining

            st.dataframe(
                raw_filtered[show_cols].reset_index(drop=True),
                use_container_width=True, height=420,
            )
            st.caption(f"Showing {len(raw_filtered):,} event(s) matching selected filters.")
...
if st.session_state.df is not None:
    ...
    for t, lbl in zip(
        ...
        with t:
            alert_banner(
                f"Upload a CSV or click <strong>🎲 Generic Demo</strong> to use the {lbl} tab.",
                "info",
            )
...
if st.session_state.tsdpl_loaded:
    ...
    for t, lbl in zip(
        [tab_ro, tab_sa, tab_pm_tab, tab_sc, tab_mm],
        ["Shift Roster","Shift Analytics","PM Checklist","Health Scorecard","Month-over-Month"],
    ):
        with t:
            alert_banner(
                f"Click <strong>🏭 TSDPL Demo</strong> in the sidebar to use the {lbl} tab.",
                "info",
            )
