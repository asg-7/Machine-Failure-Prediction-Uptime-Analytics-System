...
RISK_COLORS = {"High": "#DA1E28", "Medium": "#F1C21B", "Low": "#24A148", "Unknown": "#94A3B8"}
...
def chart_risk_ranking(predictions: pd.DataFrame) -> go.Figure:
    ...
    # Threshold lines
    fig.add_vline(x=40, line_dash="dot", line_color="#F1C21B", annotation_text="Medium")
    fig.add_vline(x=70, line_dash="dot", line_color="#DA1E28", annotation_text="High")
    return fig
...
def chart_sensor_trend(df_machine: pd.DataFrame, sensor: str, machine_id: str) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", name="UP", line=dict(color="#24A148", width=1),
    ))
    fig.add_trace(go.Scatter(
        x=down["timestamp"], y=down[sensor],
        mode="markers", name="DOWN (failure)", marker=dict(color="#DA1E28", size=5),
    ))
    # Rolling mean
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", name="Rolling Avg", line=dict(color="#0F62FE", dash="dash", width=2),
    ))
    fig.update_layout(
